...
class PartidaActivity : AppCompatActivity() {
    ...
    private var esTorneo: Boolean = false
    private var idTorneoActual: String = ""
    private var matchIdTorneo: String = ""

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        // Establecer el layout directamente
        setContentView(R.layout.activity_partida)
        esTorneo = intent.getBooleanExtra("ES_TORNEO", false)
        idTorneoActual = intent.getStringExtra("ID_TORNEO") ?: ""
        matchIdTorneo = intent.getStringExtra(EXTRA_CODE) ?: ""
        ...
        if (esTorneo) {
            roundLabel.setTextColor(Color.parseColor("#FFD700"))
            roundLabel.text = "TORNEO PARTIDA"
        }
...
// Dentro de onCreate
    ...
    private fun mostrarDialogoFinal(ganaste: Boolean) {
        ...
        runOnUiThread {
            ...
            if (ganaste) {
                ...
                verificarYNotificarGanador(if (miTalde == 1) 2 else 1)
    ...
    private fun verificarYNotificarGanador(equipoGanador: Int) {
        if (!esTorneo) return // Si no es torneo, no hacemos nada

        val prefs = getSharedPreferences("UserPrefs", MODE_PRIVATE)
        val miUid = prefs.getString("userRegistrado", "") ?: ""
        val ganadorUid = if (equipoGanador == miTalde) miUid else "RIVAL_ID"


        if (equipoGanador == miTalde) {
            db.collection("Tournaments")
                .document(idTorneoActual)
                .update(
                    "matches.$matchIdTorneo.winner", miUid,
                    "matches.$matchIdTorneo.status", "FINISHED"
                )
                .addOnSuccessListener {
                    Log.d("TORNEO", "Resultado actualizado correctamente")
                }
        }
    }
